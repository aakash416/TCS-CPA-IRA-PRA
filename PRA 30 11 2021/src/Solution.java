...
public class Solution {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < d.length; i++) {
			...
			d[i] = new Sanitizer(rackid, name, availbaleStock, price); // creating object
		}
		sc.nextLine();
	...
	public static int findtotalstock(Sanitizer[] Sanitize) {
		...
		for (int i = 0; i < Sanitize.length; i++) {
			sum += Sanitize[i].getAvailbaleStock();
...
class Sanitizer {
	...
	public Sanitizer(int rackid, String name, int availbaleStock, int price) {
		super();
		this.rackid = rackid;
		this.name = name;
